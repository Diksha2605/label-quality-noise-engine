...
def run_adult_experiment():
    ...
    print("\nTop suspicious samples (lowest trust):")
    print(
    ...
    # --------------------------------------------------
    # 9. Visual Evidence (IMPORTANT)
    # --------------------------------------------------
    plt.figure(figsize=(7, 4))
    sns.histplot(df_train["trust_score"], bins=30, kde=True)
    plt.title("Trust Score Distribution (Adult Dataset)")
    ...
    sns.scatterplot(
        x=df_train["pred_proba_max"],
        y=df_train["trust_score"],
        hue=df_train["label"],
        alpha=0.4
    )
    plt.title("Prediction Confidence vs Trust Score")
    ...
    plt.title("Trust Score by Label")
    plt.xlabel("Label")
    plt.ylabel("Trust Score")
    plt.tight_layout()
    plt.show()
